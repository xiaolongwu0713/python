...
# shift: test if STOI score can capture the overall shift in spectrogram: not sensitive to the shift.
def stoi(x, y, fs_sig, extended=False, shift=0):
    ...
    # apply a constant shift to x_spec, and test if the final score changes with this shift.
    #shift=10
    x_tob = np.sqrt(np.matmul(OBM, np.square(np.abs(x_spec)+shift)))
