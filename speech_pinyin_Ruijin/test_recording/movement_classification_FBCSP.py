'''
This script is to classify three movement: thumb up, victory gesture, tongue extrude.
Result: sid 2 and 3 all chance level;
'''
import glob
import matplotlib
...
epochs_all = mne.Epochs(raw, events_tasks, tmin=5, tmax=10,baseline=None)#.load_data().resample(500)
...
wind=200
stride=200
epoch1=np.asarray(wind_list_of_2D(epoch1,wind, stride))
epoch2=np.asarray(wind_list_of_2D(epoch2,wind, stride))
epoch3=np.asarray(wind_list_of_2D(epoch3,wind, stride))


list_of_epoch_tmp=[epoch1,epoch2,epoch3]
list_of_epoch=np.concatenate((epoch1,epoch2,epoch3),axis=0) # (100, 208, 4001)
