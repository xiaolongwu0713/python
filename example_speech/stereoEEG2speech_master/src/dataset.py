...
flags.DEFINE_string('data_dir', mydatadir, '')
...
class TacotronSTFT(torch.nn.Module):
    def __init__(self, filter_length=1024, hop_length=256, win_length=1024,  
        ...
        self.stft_fn = STFT(filter_length, hop_length, win_length).cpu() # hop and window length are in samples.
...
def get_data(split='train', hop=None):
    ...
    if FLAGS.patient_eight:
        ...
        audio=np.load(data_dir+'p'+str(sid)+'_audio_final.npy')
   ...
   #prepend data of kh4 to boost prediction of kh8
    ...
    if FLAGS.patient_eight:
        ...
        sEEG_gamma=np.load(data_dir + 'p' + str(sid) + '_sEEG_gamma.npy').T #.T only for 1khz
        eeg=sEEG_gamma
    ...
    eeg = torch.from_numpy(eeg).float() # -391.1244--262.1048
    audio = np.clip(audio / np.max(np.abs(audio)), -1, 1) # dtype('float64')
