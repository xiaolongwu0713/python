...
if socket.gethostname() == 'workstation':
    result_dir='H:/Long/data/stereoEEG2speech_master/'
elif socket.gethostname()== 'LongsMac':
    result_dir='/Volumes/second/data_local/stereoEEG2speech_master/result'
def main(_):
    ...
    class Model(pl.LightningModule):
        def __init__(self):
            ...
            if FLAGS.use_MFCCs: # 18* 80--13*80
                if not my_mel:
                    self.output_shape=self.mel_transformer.mel_spectrogram(train_ds[0][1].unsqueeze(0)).squeeze(0).T.shape #seq_len x mel bins ??
                else:
                    self.output_shape = extractMelSpecs(train_ds[0][1].cpu().numpy(), 22050, windowLength=1024/22050, frameshift=256/22050,melbins=80).shape
                if FLAGS.discretize_MFCCs:
                    class_weights=None
            ...
            if FLAGS.OLS:
                ...
                self.seq2seq = models.RNNSeq2Seq(self.input_shape, self.output_shape).double() #.to(torch.float64)
        ...
        def training_step(self, batch, batch_idx):
            ...
            x, y = batch # y:torch.Size([32, 4410])
            if FLAGS.use_MFCCs:
                # torch.Size([32, 18, 80])
                if not my_mel:
                    y=self.mel_transformer.mel_spectrogram(y).transpose(1,2) # ERROR: constant mel vale on GPU
                else:
                    y_tmp=[]
                    for i in y:
                        y_tmp.append(extractMelSpecs(i.cpu().numpy(), 22050, windowLength=1024 / 22050, frameshift=256 / 22050,melbins=80))
                    y=torch.from_numpy(np.stack(y_tmp)).to(dtype=torch.float64).to(device)
        ...
        def validation_step(self, batch, batch_idx):
            ...
            if FLAGS.use_MFCCs:
                if not my_mel:
                    ...
                    for i in y:
                        y_tmp.append(
                            extractMelSpecs(i.cpu().numpy(), 22050, windowLength=1024 / 22050, frameshift=256 / 22050,
                                            melbins=80))
                    y = torch.from_numpy(np.stack(y_tmp)).float().to(device)
    ...
    trainer = pl.Trainer(
            ...
            logger=pl.loggers.TensorBoardLogger('logs', name=result_dir + '/log'),
