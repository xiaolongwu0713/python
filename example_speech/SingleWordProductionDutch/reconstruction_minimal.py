...
if __name__=="__main__":
    ...
    for pNr, pt in enumerate(pts):
        #Load the data
        spectrogram = np.load(os.path.join(feat_path,f'{pt}_spec.npy')) # (29985, 23)
        data = np.load(os.path.join(feat_path,f'{pt}_feat.npy')) # (29985, 1143)
        if not mydata:
            labels = np.load(os.path.join(feat_path,f'{pt}_procWords.npy')) # (29985,)
        ...
        print('Random control has mean correlation of %f' % (np.mean(randomControl[pNr,:,:])))
