...
if selected_channels:
    selected_channels, acc = get_selected_channel_gumbel(sid, 10)  # 10 selected channels
else:
...
real=combined[indexes[:304],:,:]
random.shuffle(indexes)
gen=combined[indexes[:304],:,:]
a,b,c=gen.shape
more=100
noise = np.random.normal(0, 3, more*200)
m=0
for epoch in range(start,start+more):
    discriminator.train()
    generator.train()
    for i, data in enumerate(tqdm(train_loader)):
    ...
    ff = visualization([gen.transpose(0, 2, 1), real.transpose(0, 2, 1)], 'tSNE', labels,
                       display=False, epoch=epoch)
    plt.figure(ff)
    plt.savefig('del_figure2.png')
