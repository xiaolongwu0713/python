'''
This script can be better understood using this https://jalammar.github.io/illustrated-transformer/.
'''
import numpy as np
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, x, mask):
        if not extract_attention:
            ...
            # one encoder layer consists of two sublayers: this is the first sublayer
            # the first sublayer consists of one attention layer and one residual layer
            # unfold the above sublayer connection function
        ...
        # this is the second sublayer, consisting of one FF layer and one residual layer
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, x, memory, src_mask, tgt_mask):
        "There are three sublayers in the decoder layer: self-attention sublayer, encoder-decoder sublayer and FF sublayer"
        m = memory
        if extract_attention:
            # unfold the below residual connection in the else clause
            func1 = lambda x: self.self_attn(x, x, x, tgt_mask)
            func2 = lambda x: self.src_attn(x, m, m, src_mask)
            # sublayer 1 is the self attention sublayer consisting of self-attention,norm and residual.
            # TODO: should norm and func1 be swapped? because norm should be the last step.
            tmp, att_decoder = func1(self.sublayer[0].norm(x))
            x = x + self.sublayer[0].dropout(tmp)
            # sublayer 2 is the encoder-decoder attention sublayer, consisting of encoder-decoder attention, norm and residual
            tmp, att_enc_dec = func2(self.sublayer[1].norm(x))
            x = x + self.sublayer[1].dropout(tmp)
            # sublayer 3 is the FF sublayer, consisting of FF layer, norm and residual.
...
def attention(query, key, value, mask=None, dropout=None):
    ...
    d_k = query.size(-1)  # dimension of the key vector
...
class MultiHeadedAttention(nn.Module):
    def __init__(self, h, d_model, dropout=0.1):
        ...
        self.d_k = d_model // h
        self.h = h
        # there are four matrix: w_q, w_k, w_v and w_o;
    ...
    def forward(self, query, key, value, mask=None):
        ...
        # the linears are the w_q, w_k, w_v: query=x*w_q, key=x*w_k, value=x*w_v;
        query, key, value = \
            [l(x).view(nbatches, -1, self.h, self.d_k).transpose(1, 2)
             for l, x in zip(self.linears, (query, key, value))]  # query/key/value: torch.Size([50, 300, 256])

        z, self.attn = attention(query, key, value, mask=mask,
                                 dropout=self.dropout)  # x:([50, 8, 300, 32]), attn:([50batch_size, 8heads, 300, 300])

        z = z.transpose(1, 2).contiguous().view(nbatches, -1, self.h * self.d_k)  # x: ([50, 300, 256])

        return self.linears[-1](z), torch.mean(self.attn, dim=0).cpu().detach().numpy()  # x: ([50, 300, 256])
...
def make_model(src_d, tgt_d, N=6, d_model=512, d_ff=2048, h=8, dropout=0.1, norm_mel=False, encoder_only=False,
