...
import textgrid
from speech_Dutch.config import extra_EEG_extracting,extra_EEG_pairring
...
#for sid in sids:
    ...
    skip_corrupt_EEG=0
    if sid==1:
        recording_start_time=["2023/4/16 18:03:29:230","2023/4/16 18:03:29:230"]
        EEG_range.append(["2023/4/17 14:53:08:00","2023/4/17 15:05:08:00"])
        EEG_range.append(["2023/4/17 15:06:28:00", "2023/4/17 15:17:20:00"])
        audio_range=[[75038655,],[75838107,]] # no ending marking for sid1
        ...
        recording_start_time = ["2023/4/30 15:08:29:830","2023/4/30 15:08:29:830"]
        EEG_range.append(["2023/4/30 15:08:30:00","2023/4/30 15:17:32:00"])
        EEG_range.append(["2023/4/30 15:19:40:00","2023/4/30 15:28:54:00"])
        audio_range = [[22820,481658], [729925,1164061]]
        audio_length_marker=[i[1]-i[0] for i in audio_range]
        ele_count=72 # Total: 84
        bad_channels = [17,18,31,64,65,73,74,75,76,77,78,79,80]
        # first few seconds data is corrupted for both EEG recordings
        skip_corrupt_EEG = 2.5  # s
    elif sid==3:
        recording_start_time = ["2023/6/22 14:05:27:360","2023/6/22 14:05:27:360"]
        EEG_range.append(["2023/6/22 14:10:00:00","2023/6/22 14:25:00:00"])
        EEG_range.append(["2023/6/22 14:25:00:00","2023/6/22 14:38:00:00"])
        audio_range = [[622084,1048240], [1492461,1919169]]
        audio_length_marker=[i[1]-i[0] for i in audio_range]
        ...
        recording_start_time = ["2023/7/16 13:59:12:950","2023/7/16 13:59:12:950"]
        EEG_range.append(["2023/7/16 13:59:16:00","2023/7/16 14:10:00:00"])
        EEG_range.append(["2023/7/16 14:07:00:00","2023/7/16 14:16:50:00"])
        audio_range = [[86570,496675], [664098,995596]]
        audio_length_marker=[i[1]-i[0] for i in audio_range]
        ele_count=84 # Total: 84
        bad_channels = [39,40,77,80,81,82,83,84]
        # first few seconds data is corrupted for both EEG recordings
    ...
    for session in range(sessions):
        tmp1 = recording_start_time[session] # EEG recording start
        tmp2=EEG_range[session][0] # EEG clip beginning
        format = "%Y/%m/%d %H:%M:%S:%f"
        tmp1=datetime.strptime(tmp1, format)
        tmp2=datetime.strptime(tmp2, format)
        EEG_offset=tmp2-tmp1
        EEG_offset=EEG_offset.total_seconds() * 1000
        audio_offset=audio_range[session][0]

        #### read EEG data
        file = data_dir+ "P" + str(sid)+"/raw/EEG/session"+ str(session+1) + ".edf"
        ...
        offset = int(22820.0 - skip_corrupt_EEG * sf_EEG)
        ...
        if not audio_length_marker: # P1 don't have audio stop marker.
            print("No audio length: no audio ending marker: extract EEG data according to the audio length.")
        else:
            try:
                audio_error=audio_length*sf_EEG-audio_length_marker[session] # -838
                print("audio_error: "+str(audio_error)+" ms.")
        ...
        data=data[:,int(skip_corrupt_EEG*sf_EEG+offset-extra_EEG_extracting*sf_EEG):int(skip_corrupt_EEG*sf_EEG+offset+audio_length*sf_EEG+extra_EEG_extracting*sf_EEG)]
