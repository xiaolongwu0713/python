...
from sEMG_Zhangbin.dataset_LOO import sub_split, windowing, sub_split2
...
if running_from_CMD:
    ...
    target=sys.argv[3] # binary classification: ET/PD/others/NC
    test_sub=sys.argv[4]
    val_sub=sys.argv[5]
...
lr = 0.001
resume=False
mode='flatten' # pretrain/TL/resume/flatten
...
if mode=='flatten':
    result_folder = result_dir +'wind_'+str(wind_size)+'/'+ target + '/flatten/' + str(class_number) + '_classes/' + the_time.strftime('%Y%m%d') + '/'
else:
    result_folder=result_dir+'wind_'+str(wind_size)+'/'+target+'/'+str(class_number)+'_classes/'+the_time.strftime('%Y%m%d')+'/'
print("Result path: "+result_folder+".")
isExist = os.path.exists(result_folder)
if not testing and not isExist:
...
if mode=='TL':
    ...
    ET_data_tmp, PD_data_tmp, others_data_tmp, NC_data_tmp, test_data_tmp,test_task_labels_tmp, val_data_tmp,val_task_labels_tmp = \
        sub_split2(target=target,test_sub=test_sub,val_sub=val_sub)
elif mode=='flatten':
    ET_data_tmp, PD_data_tmp, others_data_tmp, NC_data_tmp, test_data_tmp, test_task_labels_tmp, val_data_tmp, val_task_labels_tmp = \
        sub_split2(target=target, test_sub=test_sub, val_sub=val_sub,flatten=True)
...
for i,epoch in enumerate(range(resume_epoch+1,resume_epoch+1+train_epochs)):
    ...
    for batch, (trainx, trainy) in enumerate(tqdm(train_loader)):
        ...
        y_pred = net(trainx)
        batchi = trainx.shape[0]
        if batchi == 1:  # only one trial in the last batch
            y_pred = y_pred[None, :]  # add dimension
        #print("y_pred shape: " + str(y_pred.shape))
        if class_number==2:
            preds = torch.from_numpy(np.asarray([0 if predi.item() <= 0 else 1 for predi in y_pred.detach()]).squeeze()).to(device) # [ 0 or 1 ]
            loss = criterion(y_pred.squeeze(), trainy.squeeze().float())  # 2:float()/4:long()
        else:
            preds = y_pred.argmax(dim=1, keepdim=True).squeeze() # Returns the indices of the maximum value of all elements in the input tensor.
        #_, preds = torch.max(y_pred, 1)
            loss = criterion(y_pred.squeeze(), trainy.squeeze().long())  # 2:float()/4:long()
        ...
        if class_number==2:
            running_corrects += torch.sum(preds == trainy)
        else:
            running_corrects += torch.sum(preds == trainy)
        if testing==True:
            break
    ...
    if epoch % 1 == 0:
        ...
        with torch.no_grad():
            for _, (val_x, val_y) in enumerate(tqdm(val_loader)):
                ...
                outputs = net(val_x)
                batchi=val_x.shape[0]
                if batchi==1: # only one trial in the last batch
                    outputs=outputs[None,:] # add dimension
                #_, preds = torch.max(outputs, 1)
                if class_number == 2:
                    preds = torch.from_numpy(np.asarray([0 if predi.item() < 0 else 1 for predi in outputs.detach()]).squeeze()).to(device)  # [ 0 or 1 ]
                else:
                    preds = outputs.argmax(dim=1,keepdim=True).squeeze()  # Returns the indices of the maximum value of all elements in the input tensor.

                #preds = outputs.argmax(dim=1, keepdim=True)

                #running_corrects += torch.sum(preds.cpu().squeeze() == val_y.squeeze())
                if class_number == 2:
                    running_corrects += torch.sum(preds == val_y)
                else:
                    running_corrects += torch.sum(preds == val_y)
...
with torch.no_grad():
    ...
    for _, (test_x, test_y) in enumerate(tqdm(test_loader)): #test_loader
        ...
        outputs = net(test_x)
        batchi = test_x.shape[0]
        if batchi == 1:  # only one trial in the last batch
            outputs = outputs[None, :]  # add dimension

        if class_number == 2:
            pred_tmp = torch.from_numpy(np.asarray([0 if predi.item() < 0 else 1 for predi in outputs.detach()])).to(device)  # [ 0 or 1 ]
        else:
            pred_tmp = outputs.argmax(dim=1,keepdim=True).squeeze(1)  # Returns the indices of the maximum value of all elements in the input tensor.
        #_, preds = torch.max(outputs, 1)
        #preds = outputs.argmax(dim=1, keepdim=True)

        #running_corrects += torch.sum(preds.cpu().squeeze() == test_y.squeeze())
        if class_number == 2:
            running_corrects += torch.sum(pred_tmp == test_y)
            running_errors += torch.sum(pred_tmp != test_y)
            test_preds.append(pred_tmp)
        else:
            running_corrects += torch.sum(pred_tmp == test_y)
            running_errors += torch.sum(pred_tmp != test_y)
            test_preds.append(pred_tmp)
...
train_result['test_task_labels']=test_task_labels
