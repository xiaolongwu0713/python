...
# LOO schema: test on one subject after training on the rest
# flatten=True: flatten 7 channels into 1 channel
def sub_split2(taski=None, target='ET',val_sub=None, test_sub=None,flatten=False): # test_on: 0:ET, 1:PD, 2:others, 3:NC; sub_id:use one perticular subject
    data_dir2=data_dir+'data2/'
    if taski is not None:#taski = [0]  # a list of task indexes
        strr = ""
        for i in taski:
    ...
    if target=='ET':
        test_data_raw = ET_data_raw[test_sub]
        test_data_labels_tmp=ET_task_labels[test_sub]
        ET_data_raw.pop(test_sub)  # remove the subject in place
        val_data_raw = ET_data_raw[val_sub]
        val_data_labels_tmp = ET_task_labels[val_sub]
        ET_data_raw.pop(val_sub)  # remove the subject in place
    elif target=='PD':
        test_data_raw = PD_data_raw[test_sub]
        test_data_labels_tmp = PD_task_labels[test_sub]
        PD_data_raw.pop(test_sub) # remove the subject in place
        val_data_raw = PD_data_raw[val_sub]
        val_data_labels_tmp = PD_task_labels[val_sub]
        PD_data_raw.pop(val_sub)  # remove the subject in place
    elif target=='others':
        test_data_raw = others_data_raw[test_sub]
        test_data_labels_tmp = others_task_labels[test_sub]
        others_data_raw.pop(test_sub)  # remove the subject in place
        val_data_raw = others_data_raw[val_sub]
        val_data_labels_tmp = others_task_labels[val_sub]
        others_data_raw.pop(val_sub)  # remove the subject in place
    elif target=='NC':
        test_data_raw = NC_data_raw[test_sub]
        test_data_labels_tmp = NC_task_labels[test_sub]
        NC_data_raw.pop(test_sub)  # remove the subject in place
        val_data_raw = NC_data_raw[val_sub]
        val_data_labels_tmp = NC_task_labels[val_sub]
        NC_data_raw.pop(val_sub)  # remove the subject in place
    ...
    test_data_labels = []
    for handi in test_data_raw:
        assert len(test_data_raw[handi]) == len(test_data_labels_tmp[handi])
        #if 'L' in test_data_raw or 'R' in test_data_raw:
        for i,taski in enumerate(test_data_raw[handi]):
            test_data_labels.append(handi+str(test_data_labels_tmp[handi][i]))
            scaler = StandardScaler()  # input shape: (n_sample, n_feature)
            taski = scaler.fit_transform((taski))
            test_data_tmp.append(taski)
    ...
    val_data_labels = []
    for handi in val_data_raw:
        assert len(val_data_raw[handi]) == len(val_data_labels_tmp[handi])
        #if 'L' in val_data_raw or 'R' in val_data_raw:
        for i,taski in enumerate(val_data_raw[handi]):
            val_data_labels.append(handi + str(val_data_labels_tmp[handi][i]))
            scaler = StandardScaler()  # input shape: (n_sample, n_feature)
            taski = scaler.fit_transform((taski))
            val_data_tmp.append(taski)

    if flatten==False:
        return ET_data_tmp,PD_data_tmp,others_data_tmp,NC_data_tmp,test_data_tmp,test_data_labels, val_data_tmp,val_data_labels
    else: # flatten channels
        print("Reading input: Flattening the data.")
        test_data_tmp2 = []
        val_data_tmp2 = []
        ET_data_tmp2=[]
        PD_data_tmp2 = []
        ...
        for i,tmp in enumerate(ET_data_tmp):
            for j in range(7):
                channeli=ET_data_tmp[i][:,j]
                ET_data_tmp2.append(channeli[:,np.newaxis])

        for i, tmp in enumerate(PD_data_tmp):
            for j in range(7):
                channeli = PD_data_tmp[i][:, j]
                PD_data_tmp2.append(channeli[:, np.newaxis])

        for i, tmp in enumerate(others_data_tmp):
            for j in range(7):
                channeli = others_data_tmp[i][:, j]
                others_data_tmp2.append(channeli[:, np.newaxis])

        for i, tmp in enumerate(NC_data_tmp):
            for j in range(7):
                channeli = NC_data_tmp[i][:, j]
                NC_data_tmp2.append(channeli[:, np.newaxis])
        ...
        for handi in test_data_raw:
            ...
            # if 'L' in test_data_raw or 'R' in test_data_raw:
            for i, taski in enumerate(test_data_raw[handi]):
                for j in range(7): # label: R_channel_task
                    test_data_labels2.append(handi+'_'+str(j)+'_'+ str(test_data_labels_tmp[handi][i]))
                    scaler = StandardScaler()  # input shape: (n_sample, n_feature)
                    tmp = scaler.fit_transform((taski[:,j][:,np.newaxis]))
                    test_data_tmp2.append(tmp)
        ...
        for handi in val_data_raw:
            ...
            for i, taski in enumerate(val_data_raw[handi]):
                for j in range(7):
                    val_data_labels2.append(handi+'_'+str(j)+'_'+str(val_data_labels_tmp[handi][i]))
                    scaler = StandardScaler()  # input shape: (n_sample, n_feature)
                    tmp = scaler.fit_transform((taski[:,j][:,np.newaxis]))
                    val_data_tmp2.append(tmp)


        return ET_data_tmp2, PD_data_tmp2, others_data_tmp2, NC_data_tmp2, test_data_tmp2, test_data_labels2, val_data_tmp2, val_data_labels2
