...
class Generator_TTSCGAN(nn.Module):
    def __init__(self, seq_len=150, channels=3, num_classes=9, latent_dim=100, data_embed_dim=10, 
        ...
        super(Generator_TTSCGAN, self).__init__()
        ...
        avgpool = nn.AvgPool2d((1, 11), stride=1, padding=(0, 5), count_include_pad=False)
        filter=nn.Conv2d(in_channels=self.channels, out_channels=self.channels, kernel_size=(1, 21), padding=(0, 10),
                  padding_mode='reflect',groups=self.channels)
        #filter.weight.data.fill_(1.0)
        #filter.bias.data.fill_(0.0001)
        self.last = filter #nn.Sequential(filter,nn.PReLU())  # nn.PReLU()

    def forward(self, z, labels):
        c = self.label_embedding(labels) # torch.Size([32, 10])
        x = torch.cat([z, c], dim=1) #torch.Size([32, 128]) -> torch.Size([32, 138])
        x = self.l1(x) # torch.Size([32, 5000])
        x = x.view(-1, self.seq_len, self.data_embed_dim) # torch.Size([32, 500, 10])
        H, W = 1, self.seq_len
        x = self.blocks(x) # torch.Size([32, 500, 10])
        x = x.reshape(x.shape[0], 1, x.shape[1], x.shape[2]) # torch.Size([32, 1, 500, 10])
        output = self.deconv(x.permute(0, 3, 1, 2)) # torch.Size([32, 5, 1, 500])

        # smooth
        output=self.last(output) # torch.Size([32, 5, 1, 500])
...
class PatchEmbedding_Linear(nn.Module):
    ...
    def forward(self, x:Tensor) ->Tensor:
        b, _, _, _ = x.shape # torch.Size([32, 5, 1, 500])
        x = self.projection(x) # torch.Size([32, 500, 50])
        cls_tokens = repeat(self.cls_token, '() n e -> b n e', b=b) # torch.Size([32, 1, 50])
