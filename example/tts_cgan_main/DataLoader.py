...
class mitbih_train(Dataset):
    def __init__(self, filename='./mitbih_train.csv', n_samples=20000, oneD=False):
        data_train = pd.read_csv(filename, header=None) # (87554, 188)
        ...
        train_dataset = pd.concat((data_0_resample, data_1_resample, 
                                  data_2_resample, data_3_resample, data_4_resample)) # (100000, 188)
        
        self.X_train = train_dataset.iloc[:, :-1].values # (100000, 187)
        if oneD:
            self.X_train = self.X_train.reshape(self.X_train.shape[0], 1, self.X_train.shape[1])
        else:
            self.X_train = self.X_train.reshape(self.X_train.shape[0], 1, 1, self.X_train.shape[1]) # (100000, 1, 1, 187)
        self.y_train = train_dataset[187].values # (100000,)
