...
class Time_GAN_module(nn.Module):
    ...
    def forward(self, x):
            ...
            if self.rnn_type in ["rnn", "gru"]:
              hidden = self.init_hidden(batch_size).type(Tensor).to(device)
            elif self.rnn_type == "lstm": # additional initial cell state for lstm
              h0 = torch.zeros(self.num_layers, x.size(0), self.hidden_dim).type(Tensor).to(device)
              c0 = torch.zeros(self.num_layers, x.size(0), self.hidden_dim).type(Tensor).to(device)
              hidden = (h0, c0)
...
def TimeGAN(data, parameters):
  ...
  for e in range(epoch): 
    for batch_index, X in enumerate(loader):
        ...
        if e in range(1,epoch) and batch_index == 0:
            print('step: '+ str(e) + '/' + str(epoch) + ', s_loss: ' + str(np.sqrt(G_loss_S.detach().cpu().numpy())))
  ...
  for itt in range(epoch):
    ...
    for kk in range(2):
      X = next(iter(loader)).type(Tensor).to(device)
      random_data #= random_generator(batch_size=batch_size, z_dim=dim, 
                                       #T_mb=extract_time(data)[0], max_seq_len=extract_time(data)[1])
        
      # Generator Training 
      ## Train Generator
      z = torch.tensor(random_data).type(Tensor).to(device)
      #z = z.float()
    ...
    for batch_index, X in enumerate(loader):
      ...
      print('step: '+ str(itt) + '/' + str(epoch) + 
            ', D_loss: ' + str(D_loss.detach().cpu().numpy()) +
            ', G_loss_U: ' + str(G_loss_U.detach().cpu().numpy()) +
            ', G_loss_S: ' + str(G_loss_S.detach().cpu().numpy()) +
            ', E_loss_t0: ' + str(np.sqrt(E_loss0.detach().cpu().numpy())))
         

      
      random_test = random_generator(1, dim, extract_time(data)[0], extract_time(data)[1])        
      test_sample = Generator(torch.tensor(random_generator(1, dim, extract_time(data)[0], extract_time(data)[1])).type(Tensor).to(device))[0]
      ...
      fig, ax = plt.subplots()
      ax1 = plt.plot(test_recovery[0].detach().cpu().numpy())
      plt.show()
