...
class BaseModel():
  ...
  def train_one_iter_g(self):
    ...
    self.X0, self.T = batch_generator(self.ori_data, self.ori_time, self.opt.batch_size)
    self.X = torch.tensor(self.X0, dtype=torch.float32).to(self.device)
    self.Z = random_generator(self.opt.batch_size, self.opt.z_dim, self.T, self.max_seq_len).to(self.device).type(Tensor)
  ...
  def train_one_iter_d(self):
    ...
    self.X0, self.T = batch_generator(self.ori_data, self.ori_time, self.opt.batch_size)
    self.X = torch.tensor(self.X0, dtype=torch.float32).to(self.device)
    self.Z = random_generator(self.opt.batch_size, self.opt.z_dim, self.T, self.max_seq_len).to(self.device)
...
class TimeGAN(BaseModel):
    ...
    def forward_g(self):
      ...
      """
      self.E_hat = self.netg(self.Z.type(Tensor))
    def forward_dg(self):
    ...
    def backward_g(self):
      ...
      """
      self.err_g_U = self.l_bce(torch.ones_like(self.Y_fake), self.Y_fake.detach()) # .detach()
      self.err_g_U_e = self.l_bce(torch.ones_like(self.Y_fake_e), self.Y_fake_e.detach())
    ...
    def backward_d(self):
      ...
      """
      self.err_d_real = self.l_bce(torch.ones_like(self.Y_real), self.Y_real.detach())
      self.err_d_fake = self.l_bce(torch.ones_like(self.Y_fake), self.Y_fake.detach())
      self.err_d_fake_e = self.l_bce(torch.ones_like(self.Y_fake_e), self.Y_fake_e.detach())
