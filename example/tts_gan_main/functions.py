...
from common_dl import Tensor
from example.tts_gan_main.utils.utils import make_grid, save_image
from tqdm import tqdm
import cv2
#from example.tts_gan_main.train_GAN import deepnet

# from utils.fid_score import calculate_fid_given_paths
from example.tts_gan_main.utils.torch_fid_score import get_fid
...
def compute_gradient_penalty_deepnet(D, real_samples, fake_samples,del_me):
    real_samples=torch.squeeze(real_samples)
    fake_samples = torch.squeeze(fake_samples)
    """Calculates the gradient penalty loss for WGAN GP"""
    # Random weight term for interpolation between real and fake samples
...
def train_del(args, gen_net: nn.Module, dis_net: nn.Module, dis_optimizer, train_loader, epoch, writer_dict, fixed_z,
            schedulers=None):
...
def train(args, gen_net: nn.Module, dis_net: nn.Module, gen_optimizer, dis_optimizer, gen_avg_param, train_loader,
    ...
    if args.deepnet == True:
        compute_gradient_penalty = compute_gradient_penalty_deepnet
    else:
        pass # use the original GP function
    ...
    for iter_idx, (imgs, _) in enumerate(tqdm(train_loader)):
        ...
        if args.deepnet==True:
            ...
            fake_imgs=torch.squeeze(fake_imgs)
        real_validity = dis_net(real_imgs)  # squeeze for deepnet

        assert fake_imgs.size() == real_imgs.size(), f"fake_imgs.size(): {fake_imgs.size()} real_imgs.size(): {real_imgs.size()}"
        fake_validity = dis_net(fake_imgs)  # squeeze for deepnet
        ...
        #  Train Generator
        # -----------------
        if epoch > 0  and global_steps % (args.n_critic * args.accumulated_times) == 0:

            for accumulated_idx in range(args.g_accumulated_times):
                gen_z = torch.cuda.FloatTensor(np.random.normal(0, 1, (args.gen_batch_size, args.latent_dim)))
                gen_imgs = gen_net(gen_z)
                if args.deepnet==True:
                    gen_imgs=torch.squeeze(gen_imgs)
        ...
        if gen_step and iter_idx % args.print_freq == 0 and args.rank == 0:
            ...
            #             save_image(sample_imgs, f'sampled_images_{args.exp_name}.jpg', nrow=4, normalize=True, scale_each=True)
            # writer.add_image(f'sampled_images_{args.exp_name}', img_grid, global_steps)
            tqdm.write("[Epoch %d/%d] [Batch %d/%d] [D loss: %f] [G loss: %f] [ema: %f] " %
