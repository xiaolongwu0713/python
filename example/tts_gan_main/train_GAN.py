...
from example.tts_gan_main import cfg
from example.tts_gan_main.dataLoader import mydataset_tts_GAN
from example.tts_gan_main.GANModels import *
from example.tts_gan_main.functions import train, validate, save_samples, LinearLrDecay, load_params, copy_params, cur_stages
from example.tts_gan_main.utils.utils import set_log_dir, save_checkpoint, create_logger
...
import warnings
from example.tts_gan_main.adamw import AdamW
import random 
...
#                 raise NotImplementedError('{} unknown inital type'.format(args.init_type))
    ...
    def init_filter(m):
        if type(m) == my_conv2d:
            ...
            m.layer.weight.data.fill_(1.0)
    gen_net = Generator(args=args)
    #gen_net.apply(init_filter)
    #print(gen_net)
    if args.deepnet==True:
        dis_net = deepnet(args.chn, 1, args.wind).to(device)
    else:
        dis_net = Discriminator(in_channels=args.chn,seq_length=args.wind)
    #print(dis_net)
...
#             dis_net = eval('models_search.'+args.dis_model+'.Discriminator')(args=args)
    ...
    train_set = mydataset_tts_GAN(args=args,norm=args.norm_method,data_mode='Train', single_class=True,classi=args.classi, augment_times=args.augment_times)
    ...
    print('Batch number per epoch: '+str(len(train_loader))) # 19 batches
...
#         gen_avg_param = list(p.cuda().to(f"cuda:{args.gpu}") for p in gen_avg_param)
    ...
    # create new log dir
        ...
        if args.rank == 0:
            args.path_helper = set_log_dir('H:/Long/data/gesture/DA/TTS-GAN/'+str(args.sid)+'/class_'+str(args.classi)+'/')
    ...
    experiment_description="Try to smooth the generated data using below method: " + "only a conv2d: self.afterpool = nn.Sequential(nn.Conv2d(in_channels=self.chn, out_channels=self.chn, kernel_size=(1,11), \
    padding=(0,5),padding_mode='reflect'))  \n Forward: \n \
        output=self.afterpool(output) \n"
...
#             train_d(args, gen_net, dis_net, dis_optimizer, train_loader, epoch, writer_dict,fixed_z, lr_schedulers)
        train(args, gen_net, dis_net, gen_optimizer, dis_optimizer, gen_avg_param, train_loader, epoch, writer_dict,fixed_z, lr_schedulers)
        for p in gen_net.asFilter.parameters():
            #p.data.clamp_(0, 1.0)
            pass
...
#         if epoch and epoch % 1 == 0:
        gen_net.eval()
        plot_buf = gen_plot(axs,gen_net, epoch, args.class_name)
...
def gen_plot(ax,gen_net, epoch, class_name):
    ...
    n_rows=gs.nrows  # return  1
    n_cols=gs.ncols  # returns 4

    for i in range(n_rows*n_cols):
        fake_noise = torch.FloatTensor(np.random.normal(0, 1, (1, 100))).to(device)
        fake_sigs = gen_net(fake_noise).to('cpu').detach().numpy() # (1, 208, 1, 500)
        synthetic_data.append(fake_sigs)
    synthetic_data=np.squeeze(np.asarray(synthetic_data),axis=1)
    ...
    for i in range(n_rows):
        if n_cols>1:
            for j in range(n_cols):
                ax[i, j].clear()
                ax[i, j].plot(synthetic_data[i * n_cols + j, 0, 0, :])
                ax[i, j].plot(synthetic_data[i * n_cols + j, 1, 0, :])
                ax[i, j].plot(synthetic_data[i * n_cols + j, 2, 0, :])
                ax[i, j].legend(['1', '2', '3'], loc="lower left", fontsize='small')

        elif n_cols==1:
            ax[i].clear()
            ax[i].plot(synthetic_data[i, 0, 0, :])
            ax[i].plot(synthetic_data[i, 1, 0, :])
            ax[i].plot(synthetic_data[i, 2, 0, :])
            ax[i].legend(['1', '2', '3'], loc="lower left", fontsize='small')
    buf = io.BytesIO()
...
#def main():
#print("args:")
#print(args.diff_aug[0])
#print(type(args.diff_aug))
args = cfg.parse_args()
