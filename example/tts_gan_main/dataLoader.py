...
from gesture.utils import read_data, windowed_data
...
class unimib_load_dataset(Dataset):
    def __init__(self, 
        ...
        adl_data = np.reshape(adl_data,(-1,num_samples,3), order='F') #uses Fortran order: (7579, 151, 3)
...
class mydataset_tts_GAN(Dataset):
    def __init__(self,args,
                 verbose=False, norm='std',
                 # False => combine test & validation groups
                 is_normalize=False,
                 one_hot_encode=True, data_mode='Train', single_class=False, classi=0, augment_times=None):
        ...
        test_epochs, val_epochs, train_epochs, scaler=read_data(self.sid,self.fs,scaler=self.norm)
        ...
        labels_train=np.array([i[0] for i in y_train.tolist()])
        labels_val = np.array([i[0] for i in y_val.tolist()])
        # tested on the class 0
        self.X_train_class, self.labels_train = X_train[labels_train==self.classi,:,:], labels_train[labels_train==self.classi] # (576, 208, 500), (576,)
        self.X_val_class, self.labels_val = X_val[labels_val == self.classi, :, :], labels_val[labels_val == self.classi]
        # format to (1524, 3, 1, 150)
        self.X_train_class=self.X_train_class[:,:,np.newaxis,:]
        self.X_val_class = self.X_val_class[:, :, np.newaxis, :]

    def __len__(self):

        if self.data_mode == 'Train':
            if self.single_class:
                return len(self.X_train_class)
        else:
            if self.single_class:
                return len(self.X_val_class)


    def __getitem__(self, idx):
        if self.data_mode == 'Train':
            if self.single_class:
                return self.X_train_class[idx,:self.chn,:,:], self.labels_train[idx]
        else:
            if self.single_class:
                return self.X_val_class[idx,:self.chn,:,:], self.labels_val[idx]
