...
import h5py
import pandas as pd
from gesture.group_analysis.util import reduce_duplicated_hyph
...
good_channels_all=get_good_channels()
reg={}
ele={}
...
for i,j in enumerate(info[:,0]):
    ...
    reg[sid] = list(tmp['CHN'][:, 0]) # sid2: 121 channel index in a list, starting from 1(0th element);
    reg_tmp=[c-1 for c in reg[sid]]
    #tmp=good_channels_tmp[i][0][sid][0][0,:]
    #tmp_list=[int(m) for m in tmp]
    #good_channels[sid]=tmp_list # sid2: 115 channels
    good_channels=good_channels_all[sid]
    good_channels=[c-1 for c in good_channels]
    good_channels_indices=[reg_tmp.index(int(l)) for l in good_channels] # 115 good channel index in the registration file
    ...
    for key in ['ana_label_name', 'name']:
        ...
        for c in range(channel_number):
            ...
            a=f[f['elec_Info_Final_wm'][key][c,0]]
