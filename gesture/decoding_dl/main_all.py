...
if running_from_CMD: # run from cmd on workstation
    if socket.gethostname() == 'workstation' or socket.gethostname() == 'DESKTOP-NP9A9VI':
        ...
        wind = int(float(sys.argv[4])) #500
        stride = int(float(sys.argv[5])) #200
        try:
            retrain = sys.argv[6] # with_selected_channels/DA
        except IndexError:
            retrain = False
        try:
            method = sys.argv[7] #'gumble'/'stg'/'mannual' or 'VAE'/'GAN'/'CWGANGP'
        except IndexError:
            method = False
        try:
            gen_epochs=sys.argv[8]
        except IndexError:
            gen_epochs=False
    ...
    wind = 500
    stride = 200
    retrain='False' #'DA'/False/'selected_channels'
    method='TTSCGAN' # 'NI'/'VAE'/'CWGANGP'/
    gen_epochs=200
class_number=5
...
if retrain=='selected_channels':
...
elif retrain=='DA': # data augmentation
    ## TODO ad hoc
    channel_num_selected = 10
    selected_channels, acc = get_selected_channel_gumbel(sid, channel_num_selected)
    ...
    if method=='NI':
        ...
        model_path = result_path
    elif method in ['CWGANGP','TTSCGAN']:
        print("Python: Re-train with augmentated data generated by "+method+" with "+str(gen_epochs)+" epochs.")
        #selected_channels = False
        result_path = result_dir + 'deepLearning/retrain/DA_'+method+'/'+str(sid)+'/'
        model_path = result_path
        gen_data_all = read_gen_data(sid,method,gen_epochs,'std')  # (760, 5, 500) * 5
    else:
        print(r"Method should be one of ['NI','CWGANGP','TTSCGAN']")
elif retrain=='False':
    gen_data_all=None
    selected_channels = False
    result_path = result_dir + 'deepLearning/vanilla/' + str(wind)+'_'+str(stride)+'/'+str(sid) + '/'
    model_path=result_path
    print("Train using all channels.")
...
X_train,y_train,X_val,y_val,X_test,y_test=windowed_data(train_epochs,val_epochs,test_epochs,wind,stride,
                                                        gen_data_all=gen_data_all,retrain=retrain,method=method)
...
if model_name=='eegnet':
    ...
    net=d2lresnet(class_number,end_with_logsoftmax=False) # 92%
