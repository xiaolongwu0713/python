...
class EmbeddingNetwork(torch.nn.Module):
    ...
    def forward(self, X, T):
        ...
        X_packed = torch.nn.utils.rnn.pack_padded_sequence(
            input=X,  # torch.Size([128, 100, 6])
            lengths=T, # torch.Size([128])
            batch_first=True, 
        ...
        # 128 x 100 x 10
        logits = self.emb_linear(H_o) #torch.Size([128, 100, 20]) -> torch.Size([128, 100, 20])
        # 128 x 100 x 10
...
class TimeGAN(torch.nn.Module):
    ...
    def _recovery_forward(self, X):
        ...
        """
        # Forward Pass
        T=[500,]*X.shape[0] # len: 32
        H = self.embedder(X, T) # torch.Size([32, 500, 100])
        X_tilde = self.recovery(H, T) # torch.Size([32, 500, 190])

        # For Joint training
        H_hat_supervise = self.supervisor(H, T) # torch.Size([32, 500, 100]) --> torch.Size([32, 500, 100])
    ...
    def _generator_forward(self, X, Z, gamma=1):
        ...
        T = [500, ] * X.shape[0] # list: 32
        H = self.embedder(X, T)
        H_hat_supervise = self.supervisor(H, T)

        # Generator Forward Pass
        E_hat = self.generator(Z, T) # torch.Size([32, 500, 190]) --> torch.Size([32, 500, 100])
        ...
        G_loss_gen=G_loss_U+G_loss_U_e
        # 2. Supervised loss
        ...
        # 4. Summation
        #G_loss_sum = G_loss_U + gamma * G_loss_U_e + 100 * torch.sqrt(G_loss_S) + 100 * G_loss_V
        G_loss_sum = G_loss_U + gamma * G_loss_U_e  # + 100 * G_loss_V

        return G_loss_sum,G_loss_gen,torch.sqrt(G_loss_S)
