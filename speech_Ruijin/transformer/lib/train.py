...
from speech_Ruijin.transformer.lib.eeg_transformer import *
...
class Batch:
    def __init__(self, src, trg=None, pad=0, device=device,encoder_only=False):
        ...
        # TODO: should be triangle shape as the target mask ?
...
def run_epoch(epoch,batch_number,writer,data_iter, model, loss_compute):
    total_loss = 0
    for i, batch in enumerate(data_iter):
        out, att_encoder, att_decoder, att_enc_dec = model.forward(batch.src, batch.trg, # src:torch.Size([50, 300, 10]); trg:torch.Size([50, 299, 2])
                            batch.src_mask, batch.trg_mask) #:batch.src_mask: all 1s; batch.trg_mask:torch.Size([50, 299, 299])
        ''' # att_encoder: 6*[(8, 151, 151)]; att_decoder:6*(8, 99, 99); att_enc_dec:6*[(8, 99, 151)]
        ...
        total_loss += loss
        if model.training:
            writer.add_scalar('train_loss', loss, epoch*batch_number + i)
        else:
            writer.add_scalar('val_loss', loss, epoch * batch_number + i)
    return total_loss / (i+1), out, batch.trg_y ,att_encoder, att_decoder, att_enc_dec # out: torch.Size([50, 199, 2])
...
def output_prediction(model,ts_test,ls_test, max_len, start_symbol,output_d,device,encoder_only=False):
    ...
    memory,att = model.encode(src.float(), src_mask)
